...
def test_intake_accepts_then_deduplicates(monkeypatch, tmp_path):
    ...
    with TestClient(app) as client:
        accepted = client.post("/v1/trajectory/events", json=_fixture("valid.json"))
        duplicate = client.post("/v1/trajectory/events", json=_fixture("valid.json"))
...
def test_intake_quarantines_invalid_delivery(monkeypatch, tmp_path):
    store = TrajectoryStore(tmp_path / "trajectory.sqlite3")
    monkeypatch.setattr(trajectory_api, "get_trajectory_store", lambda: store)
