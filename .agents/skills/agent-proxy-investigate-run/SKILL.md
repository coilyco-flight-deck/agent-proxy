...
Use the repo-owned read-only query helper. It joins metadata-only operational views without granting authority or exposing raw bodies.

1. Run `just trajectory-query investigate` with one or more exact `--repository`, `--issue`, `--workflow`, or `--trajectory` filters.
2. Report the matched trajectory count, view freshness, missing evidence, reliability, cost and latency, rules, evaluation, dossier, and trace joins.
