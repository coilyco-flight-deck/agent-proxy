...
* **Logical model routing** - landed - logical names (`fast-think`, `fast`, `ctx-think`, `ctx`, `tune`) map to a backend plus that model's safe `num_ctx` (`fast-think` = 49152, others 32768 until the leg-03 benchmark locks values). (leg 02 logical routing)
